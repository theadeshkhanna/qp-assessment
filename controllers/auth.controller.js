...
const login = async (req, res) => {
    ...
    try {
        ...
        const isPasswordCorrect = await authService.comparePasswords(payload.password, dbUser.password)
...
const signup = async (req, res) => {
    ...
    try {
        ...
        const dbUser = await userService.findUserByEmailId(payload.email)
        if (dbUser) {
            return res.status(404).json({ error: `user with this email already exists, please login` })
        }
